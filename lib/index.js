...
if (process.env.CI == "true") {
    ...
    additionalIssueLabels = core.getInput("additional_issue_labels").split(",").map(x => x.trim());
    ...
    for (let i = 0; i < launchArgs.length; i++) {
        if (launchArgs[i] === "--owner_source") {
            ...
            additionalIssueLabels = launchArgs[i + 1].split(",").map(x => x.trim());
        }
...
gitHubSource.getIssue(number).then((response) => {
    ...
    if (ONLY_SYNC_ON_LABEL && !labels.find(label => label === ONLY_SYNC_ON_LABEL))
        return;
        ...
        case "issues":
            ...
            switch (payload.action) {
                case "opened":
                    ...
                    gitHubTarget.createIssue(issue.title, issue.body, labels)
                        .then((response) => {
                ...
                case "unlabeled":
                    gitHubTarget.getIssueNumberByTitle(issue.title).then((targetIssueNumber) => {
                        if (targetIssueNumber) {
                            ...
                            gitHubTarget.editIssue(targetIssueNumber, issue.title, issue.body, issue.state, labels)
                                .then((response) => {
